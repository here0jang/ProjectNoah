...
public class ItemDatabaseEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Add Item"))
        {
            ...
            source.AddItem(newItem); // newItem 을 List<Item> 에 저장
        }
    ...
    void DrawItemEntry(SerializedProperty item) // 아이템 총 개수가 필요함 ( item.arraySize )
    {
        ...
        if(GUILayout.Button("x", GUILayout.Width(20f)))
        {
            ...
            s_itemsNames.DeleteArrayElementAtIndex(item.FindPropertyRelative("itemId").intValue); // 배열 공부 다시 하고 싶어짐, 배열의 원소 삭제 방법 등등...
    ...
    void ReCaluculateID() /* 배열 삭제법 관련해서 다시 공부해서 알고리즘 다시짜기~~~ */
    {
        for(int i=0; i<s_items.arraySize; i++)
        {
            ...
            s_itemsNames.GetArrayElementAtIndex(i).stringValue =
